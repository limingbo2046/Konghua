...
namespace ParrotMimicry
{
    ...
    public partial class PlayVideoPage : ContentPage
    {
        ...
        public PlayVideoPage(string videoPath)
        {
            InitializeComponent();
        ...
        private void LoadVideo(string videoPath)
        {
            ...
            LoadNotes(videoPath.Replace(".mp4", ".txt"));
            mediaElement.Play();
        ...
        private void LoadNotes(string filePath)
        {
            ...
            using (StringReader reader = new StringReader(notes))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    var sub = Subtitles.FirstOrDefault(p => p.Index == index);
                    if (sub != null)
                    {
                        sub.Note = reader.ReadLine();
                    }
                }
            }
        }
        ...
        {
            saveNotes(_videoPath);
            Navigation.PopModalAsync();
        }

        private void saveNotes(string? videoPath)
        {
            ...
            {
                if (string.IsNullOrEmpty(sub.Note))
                {
                    continue;
                }

                notes.AppendLine(sub.Index.ToString());
                notes.AppendLine(sub.Note);
            }

            if (notes.Length > 0)
            {
                File.WriteAllText(videoPath.Replace(".mp4", ".txt"), notes.ToString());
            }
